...
当前工程见 [models/malenia](../../models/malenia/README.md)。最新 `reference_motion_v7` 保留精细几何和 Minecraft 像素贴图，当前为 91 骨骼、378 cube、512×512 合并像素图集及 40 段动画。首先纠正旧模型的解剖左右反置：以角色面向 -Z 为准，Blockbench 的 +X 是角色右侧，-X 是左侧；几何、枢轴、UV 面和动画横向旋转同步修正，保证右义手持刀、左手空手、左腿义肢，不能只凭骨骼名称判定左右。空手新增八根近节/末节指骨和两根拇指骨，待机放松、抓取前张开、接触后收拢、释放或落空时放开；保留右手闭合握柄、腕部限幅补偿、头顶覆盖、精确关节和脚步相位。本体 1.2 倍制作比例、花朵、碰撞箱、伤害、技能时序与真实移动速度不变。

在前两项修正完成后，本轮通过 B 站和国内必应检索，实际取帧检查苍星化羽的原版全招式展示与谜之灭的拆解视频，未访问谷歌或 YouTube。按[动作优化计划](../../models/malenia/MOTION_PLAN.md)区分高低刀路、贴身突刺蓄力、上挑悬停下劈、踢击回收、抓取前伸、水鸟头顶横刀与不同突进姿态，以及二阶段高翼悬停、幻影后的本体突刺和艾奥尼亚收身俯冲。审查同时纠正了躯干前倾方向和肩部持刀高度。没有获得可核实授权的动作解包数据，当前是低清公开视频指导的原创人工适配，不承诺原作关节轨迹或时序逐帧一致，参考画面不进入导出资源。

进入游戏前完成 88 个角度、16 张审查图、27 张标准视图和 14 张动作分镜的审查；32 个表面审计姿势未检出外露近共面冲突，6 组遮蔽内部接触单独记录。405 条头顶射线、264 个可见握持采样、3,250 个腕部补偿采样、11 个实际刀路/重心姿态检查、1,790 条步态检查及 113 条时长/事件/方向合同检查通过，6 个隐藏握持姿态单独排除。Java 表现代码本轮未改动，双平台编译与资源处理通过；NeoForge 已实际进入世界并正常退出，用户确认左右手和空手正常，主要动作更接近参考；Forge 本轮未重复进世界。该实装口径覆盖第 3 节建议尺寸、第 4 节局部密度和第 5 节贴图分组建议，不依赖发光层。以上不是所有帧、视角、地形与网络条件的穷尽证明，逐招、多人、高延迟与完整战斗验收仍独立记录。竞技场、独立幻影、持久花朵、刀轨、粒子、UI 和音频不因本次模型交付而自动完成，下表仍是完整表现合同。
