...
2026-09-11 模型制作更新：当前[约定之王工程](../../models/promised_consort/README.md)为 `reference_lion_v3`，125 根骨骼、550 个 cube、3208 个可导出面、512×512 原创像素图集与 43 段动画。拉塔恩 364 个 cube，米凯拉与发幕 186 个 cube，均在原预算内。加深面甲遮挡，米凯拉改为肩后前倾、偏侧头位与紧凑环抱；自然关节、段内衔接和施法速度动画映射已重做。Forge 使用 GeckoLib 4.8.4，NeoForge 使用 4.9.2；本体与分身沿用原 geo/animation 合同及独立材质。v2 实机动作未通过，v3 已获得 Forge 用户对所见动作和着色器效果的改善确认；双端实机范围与未验项目以模型工程记录为准，不继承旧版本验收。
...
完整台词只显示字幕，不制作米凯拉或拉塔恩的对白录音，也不要求口型、头部配合、四臂呼吸或额外模型动画。本轮只注册独立瞬防提示音并使用原版声音占位；拉塔恩的入场/胜利战吼、喘息、受击和倒地事件及配置合同继续保留，但等待原创或明确授权 OGG 后再注册和播放。完整事件、说话者、字幕文本与语言资源键以[Boss 台词字幕与非语言音效规范](../dialogue/boss-voice-lines.md)为准。
