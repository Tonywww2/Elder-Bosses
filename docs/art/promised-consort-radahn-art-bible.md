...
当前实装先固定 GeckoLib 资源与动画合同，不制作最终网格：Forge 1.20.1 使用 GeckoLib 4.8.4，NeoForge 1.21.1 使用 4.9.2；Boss 本体和分身均使用合法空 geo 与空纹理。animation JSON 预建待机、移动、受击、眩晕、死亡、阶段转换和全部 22 个动作 ID 的空 clip，状态机按正式 ID 驱动，后续模型交付时只替换骨骼资源与关键帧。空模型阶段不持续绘制碰撞箱、轮廓或脚下定位环。
...
- 技能表现使用原版粒子占位，服务端 `[promised_consort.visuals]` 控制总开关与 `minimal`、`reduced`、`full` 三档质量，并服从单 Boss 粒子预算。
- 重力岩块使用原版方块模型与重力粒子，默认方块为 `minecraft:crying_obsidian`，资源 ID 可配置。
- 分身是无碰撞、不可选取、不可受伤的 GeckoLib 空模型表现实体，最多同时 4 个；攻击路径与命中仍由服务端快照和指示器表达。
...
最终 Boss 模型和竞技场建筑均等待用户后续提供；本节继续保留完整资产合同，后续接入时按表验收。当前只创建 GeckoLib 空骨架、完整空动画 clip、原版粒子/方块表现和纯色物品纹理，不创建竞技场 NBT 或临时替代建筑。
