...
> 最后核对：2026-09-16（本次补充建筑研究，未重验战斗数值）
...
### 4.1 2026-09-16 建筑多角度补充研究

用户明确批准本轮联网检索、查看和下载内部研究素材，并要求神之门更宏伟、尽可能参考原游戏多角度场地还原。以下视频通过公开匿名流抽样，不登录、不绕过限制；请求清晰度档位 32，实际返回 16，不能声称取得高清或逐帧精确尺寸。每张图的来源、作者、发布时间和取帧区间记录在同名 JSON 中。
...
- 第 8 张正面与赏析约 03:20 的全门画面显示：主体是厚重的两片门壁，外侧顶缘较高，朝中央形成不规则下落的断口；不是尖塔、规则圆拱或跨接横梁。
- 赏析约 00:20、00:40、02:20 以及探索约 00:24、00:36 可辨认根部展开、纵向凹脊、中央窄通路与前阶关系；门脚不是独立细柱落在一条小基座上。
- 探索约 01:48、02:00、02:12 说明门体不能当作三格薄片处理，但含越界视角，不能据此复制未完成背面、底面缺面或不可达平台。
- 第 11 张图提供南侧接近路线宽阶与窄踏步的节奏参考，不把它混同为神之门脚下的短阶，也不由截图推算原作绝对格数。
- 赏析约 01:00、01:40 的孤立局部展示和约 03:00 的对照画面不用于场地布局；探索中的天空、地板底面和穿模片段不用于正常碰撞设计。

对应改编：用户在查看研究结论后批准门体试稿 27×36×11，`X=-13..13, Y=5..40, Z=-60..-50`，约 43 格宽的根部，真实短阶前缘仍为 `Z=-42`。该数值是项目施工选择，不是原作实测；半径 40 战区、回归点、实际技能范围、南入口及祭台不变。表面细节继续用原创风化石与抽象根状浮雕表达，不提取原游戏网格、贴图或局部模型。旧白模完整保留，新版仍待视觉与进世界验收。
