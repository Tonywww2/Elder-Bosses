...
- 当前资产范围：Boss 最终模型暂缓，当前实装使用合法空模型占位；竞技场建筑暂缓，当前实装不创建竞技场 NBT、不实现建筑放置链路，也不生成临时替代建筑。模型与建筑设计完整保留在 `art/` 与 `arenas/` 文档中。
- 最终程序边界：招式、阶段、时间轴、位移与判定由 Java 程序控制；后续提供的 Boss 建筑使用原版数据包 NBT 结构模板，Java 只负责模板放置、标记解析、战斗边界、破坏记录与恢复。
- 配置策略：项目只注册 `config/elder_bosses-common.toml`；生命、多人缩放、抗性、选招权重、时序、判定、伤害、硬直、瞬间防御、逻辑场地规则、破坏预算、奖励数值与特效预算均位于该 common 配置。建筑几何、方块、材料和锚点属于 NBT 资源合同，不可由 TOML 改写。
