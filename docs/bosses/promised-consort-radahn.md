...
最终竞技场将由服务端从数据包放置 NBT 结构模板，建筑资产表现为半径 40 格、净高至少 28 格的圆形灰白荒原。半径 0-34 格保持近似平坦，34-40 格才进入低缓边界；地面模拟原作的浅层风化石与灰烬，任意 5×5 格范围高差不超过 1 格。北侧 36 格处建立神之门与高台剪影，南侧 40 格设置雾门和入场通道。神之门承担背景、阶段转换锚点和 Boss 回归锚点，不作为可卡位掩体。建筑资产和放置链路不属于当前实装范围，当前不创建临时场地；模板 ID、分片、数据标记、坐标和后续放置流程仍完整保留在[约定之王竞技场 NBT 结构规范](../arenas/promised-consort-arena.md)。
...
预测落点采用目标当前位置加最近 10 tick 平均速度 × 12 tick。落点中心限制在距场心 27 格内，即 `logical_radius 40 - outer_radius 13`，保证完整外环始终位于竞技场内；警示出现后不再修正。核心伤害为 `8 + 120%（物理）`，外环伤害为 `4 + 65%（物理）`，随后向外扩张的余波伤害为 `2 + 30%（魔法，圣属性来源）`。核心和外环互斥，玩家不会在同一 tick 同时承受两者。
...
配置中的 `flat` 是固定值，`attack_ratio` 是攻击力比例；上例在设计表中显示为 `4 + 80%` 与 `3 + 55%`。`damage_routing` 只决定是否进入护甲结算，突刺和圣属性等来源倍率继续由 `source_multiplier` 配置。硬直比例、硬直上限、距离档位、瞬间防御窗口、字幕时机、非语言音效音量/冷却和全部技能数值均由 common TOML 控制；逻辑战斗半径、方块标签和穿墙参数作为后续竞技场接入配置保留；建筑尺寸、方块、材料和锚点由未来提供的 NBT 结构资源固定；指示器透明度只在根节 `[indicators]` 出现一次。每个技能必须暴露 `enabled`、`weight`、`cooldown_ticks`、所有伤害分量和判定范围。启动时校验比例不小于 0、距离档位严格递增、字幕与战吼 tick 位于对应状态内、范围不超过逻辑战斗直径、动作时长不小于 1；非法值回退到默认值并记录明确日志。
...
| 场地构造 | 逻辑半径 40、平坦核心半径 34；灰白荒原、北侧神之门、南侧雾门、场心与回归锚点符合构造规范；任意 5×5 格高差不超过 1 格 |
