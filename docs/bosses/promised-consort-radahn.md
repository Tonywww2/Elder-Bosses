...
分身是无碰撞、不可选取、不可受伤的 GeckoLib 空模型表现实体，同时最多存在 4 个；真正判定由 Boss 动作时间轴创建。分身命中只造成表中标明的较低伤害，不复制 Boss 完整攻击。正式模型到位前只显示对应攻击路径、指示器和可配置原版粒子占位。
...
生命首次跨过 25% 时，默认把本次伤害截断到 25% 并丢弃溢出；当前 `ACTIVE` 尚未结束的等待期锁血免伤，随后强制进入 `METEOR_SCRIPT`。脚本总长 150 tick，所有 tick 从 0 开始：0-50 tick 起跳并逐渐隐去，51-90 tick 场地出现星图和移动方向相反的尘流，91 tick 锁定预测落点，110 tick 起显示核心圈和外环，121 tick 落地结算。Boss 在 0-120 tick 无敌，整个脚本不接受硬直积累；进入脚本时清除此前全部 Boss 所有危险区。

预测落点采用目标当前位置加最近 10 tick 平均速度 × 12 tick。落点中心限制在距场心 27 格内，即 `logical_radius 40 - outer_radius 13`，保证完整外环始终位于竞技场内；警示出现后不再修正。核心伤害为 `8 + 120%（物理）`，外环伤害为 `4 + 65%（物理）`，随后向外扩张的余波伤害为 `2 + 30%（魔法，圣属性来源）`。核心和外环互斥，目标不会在同一 tick 同时承受两者。首次脚本完成后默认进入 3600 tick 冷却；冷却结束会在当前 `ACTIVE` 结束后再次强制排队，不进入普通随机候选池。
...
动画层固定使用 GeckoLib 4：Forge 1.20.1 使用 4.8.4 并补 `mclib:20` 开发运行库，NeoForge 1.21.1 使用 4.9.2。约定之王实体实现 `GeoEntity`，当前提供合法空 geo、空纹理和覆盖待机、移动、受击、眩晕、死亡、转换及 22 个动作 ID 的空 animation clip；后续正式模型只替换资源和关键帧。GeckoLib 不写入模组 metadata，缺失时接受类加载失败。核心状态机不从动画回调决定伤害。
...
- 不创建竞技场 NBT、不实现模板加载/放置/标记解析/场地快照或建筑恢复；完整建筑设计继续保留，等待用户后续提供资产。
- 大剑与光冠仅为普通物品占位；配方、装备属性、主动技能和重复挑战方式后置。
