...
| 战斗台词 | 入场由拉塔恩战吼，转阶段与战败由米凯拉承担叙事 | 保留角色分工；米凯拉只显示原创字幕，拉塔恩战吼保留非语言音效 |
...
每个生命伤害事件必须声明结算通道。常规物理、突刺和流血触发伤害统一为`物理`，进入 Minecraft 护甲、抗性与附魔结算；魔法、圣属性和霜冻触发伤害统一为`魔法`，绕过护甲，但仍接受魔法通道倍率与其他合法减伤。原来源标签仅用于特效、积累和来源倍率，不再创建独立伤害类型。火焰、雷电、毒、凋零和睡眠规则保持独立。
...
战斗锁定时记录竞技场内非旁观、存活且非创造模式的玩家数 `N`，`N` 限制在 1-4。默认只缩放耐久：
...
玩家中途离开不会降低当前最大生命；新玩家穿过雾门时按差额增加最大生命和当前生命，防止通过进出竞技场刷低血量。
...
| 中毒/凋零 | 0.35 | 0.35 | 持续效果强度倍率，不免疫 |
| 流血 | 0.50 | 0.50 | 积累倍率；触发的生命伤害进入物理通道 |
| 霜冻 | 0.50 | 0.50 | 积累倍率；触发的生命伤害进入魔法通道，减速不影响关键受控位移 |
| 睡眠 | 0.00 | 0.00 | 完全免疫 |

## 4. 战斗空间与目标规则

竞技场由服务端程序构造为半径 28 格、净高至少 20 格的圆形灰白荒原。地面模拟原作的浅层风化石与灰烬，任意 5×5 格范围高差不超过 1 格；北侧 24 格处建立神之门与高台剪影，南侧设置雾门和入场通道。神之门承担背景、阶段转换锚点和 Boss 回归锚点，不作为可卡位掩体。完整坐标、材质分层和构造顺序见[约定之王竞技场构造规范](../arenas/promised-consort-arena.md)。

拉塔恩可按动作需要破坏白名单方块并短时忽略墙体碰撞；神之门骨架、雾门、竞技场边界和结构锚点始终受保护。地面不因破坏形成超过 1 格的永久坑洞，战斗重置时恢复本场被破坏的方块快照。
...
| 远距 | 14-28 格 | 重力陨石、入场突击、追近 |

Boss 每 20 tick 重新评估主目标。默认分数为距离权重 50%、最近 8 秒输出权重 35%、正在使用食物或药水的机会权重 15%。同一玩家连续成为主目标 200 tick 后，其评分乘 0.75，鼓励多人换仇恨。所谓「读指令」只读服务端可观察状态，不读取客户端按键。
...
| `INTRO` | 60 tick | 否 | 受控 | 锁定玩家、关闭雾门、执行重力入场；第 18 tick 播放拉塔恩战吼 |
| `PHASE_1` | 动态 | 是 | 是 | 双剑与重力招式池 |
| `TRANSITION` | 140 tick | 否 | 受控 | 清投射物、米凯拉附身、回到场心 |
| `PHASE_2` | 动态 | 是 | 是 | 一阶段招式、圣光回响、米凯拉招式 |
| `METEOR_SCRIPT` | 150 tick | 部分时段否 | 受控 | 25% 生命脚本化大荒星陨 |
| `STUNNED` | 80 tick | 是 | 否 | 打断并清除当前动作；可正常受伤，无特殊伤害倍率 |
| `DEFEATED` | 160 tick | 否 | 否 | 第 24 tick 显示米凯拉战败字幕，随后清理实体、结算掉落、开启出口 |
...
独立技能使用硬冷却，基础连段使用动作组软冷却。所有权重和冷却均进入配置文件。
...
- 重力陨石使用可破坏的投射物实体，存活 60 tick；转向角每 tick 不超过 4°，允许横向冲刺摆脱。
- 血焰横扫在地面留下 24 tick 裂痕，第 16 tick 爆燃。裂痕不破坏方块，不点燃场外实体。

## 8. 阶段转换

当生命首次降至最大生命的 65% 或以下时，当前技能进入 10 tick 强制收招，然后切换 `TRANSITION`：
...
转换演出不会重置异常积累、仇恨或 Boss 冷却。回归落点若被方块阻挡，服务端使用预先验证的场心锚点，不动态寻找玩家脚下位置。
...
| `CONSORT_METEOR` 大荒星陨 | 90 / 12 / 48 | 落点半径 9 格，外环至 13 格 | 核心 `8 + 120%（物理）`；外环 `4 + 65%（物理）`；余波 `2 + 30%（魔法，圣属性来源）` | 不可 | 3600 |
...
分身是无碰撞、不可选取、不可受伤的表现实体；真正判定由 Boss 动作时间轴创建。分身命中只造成表中标明的较低伤害，不复制 Boss 完整攻击。
...
| `CLONE_METEOR_4` | 重力陨石二阶段派生 | 陨石结束后 4 个分身连续下砸，本体拔剑产生光柱 | 紫色陨石尾迹先清空，再按四拍显示金色落点线，禁止两套噪声叠色 | 先横跑陨石，再按固定节奏连续换向 |
...
生命首次降至 25% 时，Boss 完成当前 `ACTIVE` 段后强制进入 `METEOR_SCRIPT`。0-50 tick 起跳并逐渐隐去；51-90 tick 场地出现星图和移动方向相反的尘流；91 tick 锁定预测落点；110 tick 起显示核心圈和外环；121 tick 落地结算。

预测落点采用目标当前位置加最近 10 tick 平均速度 × 12 tick，并限制在竞技场内。警示出现后不再修正。核心伤害为 `8 + 120%（物理）`，外环伤害为 `4 + 65%（物理）`，随后向外扩张的余波伤害为 `2 + 30%（魔法，圣属性来源）`。核心和外环互斥，玩家不会在同一 tick 同时承受两者。
...
所有可瞬间防御的攻击段必须在命中前同时发出明显红色武器轮廓/方向脉冲和 `elder_bosses:promised_consort.instant_guard_cue` 独立音效。多段连击的每个可瞬防攻击段都要独立提前至少 6 tick 提示，不能用第一段提示覆盖后续攻击。低粒子模式不能关闭红色轮廓；静音玩家可凭光效判断，色觉障碍玩家可凭音效与三段式脉冲节奏判断。红光只表示可瞬间防御，不能与血焰裂痕、重力紫光或米凯拉金光共用形状。

瞬间防御不直接增加 Boss 硬直值，也没有独立累计次数。分身、重力陨石、震地、尖刺、地裂、碎屑波、血焰爆燃、圣光回响、光柱、圣环和大荒星陨不可瞬间防御。任何连段连续三次被普通格挡或瞬间防御后，Boss 必须进入至少 20 tick 可行动后摇。
...
硬直值达到上限后立即取消当前动作并进入 80 tick `STUNNED`：前 20 tick 跪地，21-60 tick 保持眩晕，61-80 tick 起身。眩晕期间不能移动、选招或攻击，但仍可正常受伤；不存在胸口交互点、额外伤害倍率或专用攻击动作。霸体动作不免除硬直积累，只把进入 `STUNNED` 延迟到当前 `ACTIVE` 段结束，随后取消剩余动作。眩晕结束后硬直值清零，并获得 100 tick 硬直积累免疫。转阶段保留当前硬直值与上限。
...
- `elder_bosses:arena_protected`、神之门骨架、雾门、竞技场边界、基岩、方块实体和未列入白名单的方块不可破坏。每 tick 最多破坏 40 个方块，脱战或重置时恢复本场由 Boss 破坏的方块快照。
- 穿墙只改变 Boss 与方块碰撞，不提供无敌、不穿过玩家盾牌面，也不允许离开竞技场边界；出口无合法站立点时取消动作并回到最近合法路径点。
- 连续 20 tick 水平位移小于 0.05 格且目标可见时判定卡住；先尝试受控穿墙或破坏白名单方块，失败后取消动作并回场心。
...
台词选择和触发状态由程序固定，TOML 只控制启用、触发 tick、统一持续时间和队列数量。对白不制作 OGG，不要求米凯拉或拉塔恩的口型和额外模型动画。拉塔恩战吼、呼吸、受击和倒地短音继续使用原创或明确授权 OGG；瞬间防御提示音优先于这些非语言战斗音效。
...
配置中的 `flat` 是固定值，`attack_ratio` 是攻击力比例；上例在设计表中显示为 `4 + 80%` 与 `3 + 55%`。`damage_routing` 只决定是否进入护甲结算，突刺和圣属性等来源倍率继续由 `source_multiplier` 配置。硬直比例、硬直上限、距离档位、瞬间防御窗口、字幕时机、非语言音效音量/冷却、场地尺寸、方块标签和全部技能数值均由 common TOML 控制；指示器透明度只在根节 `[indicators]` 出现一次。每个技能必须暴露 `enabled`、`weight`、`cooldown_ticks`、所有伤害分量和判定范围。启动时校验比例不小于 0、距离档位严格递增、字幕与战吼 tick 位于对应状态内、范围不超过竞技场直径、动作时长不小于 1；非法值回退到默认值并记录明确日志。

招式选择、连段分支、动作时间轴、位移曲线、命中窗口、瞬间防御段、红光/音效提示触发点、阶段转换和陨星脚本全部由 Java 程序控制，不引入自定义招式数据包类型。原版仅用于配方、战利品表、标签、模型、动画、纹理、声音和语言文件等既有资源类型。TOML 只提供可调数值，不能新增技能、改变程序状态图或注入任意执行逻辑。资源位置统一使用 `elder_bosses:promised_consort/...`；配置重载只在未开战时替换模板，进行中的实例继续使用开战快照。
...
```text
src/main/java/.../boss/      共享状态机、伤害公式、技能数据模型与测试向量
...
src/test/                    纯 Java 行为测试与两端共享测试向量
```

动画层可采用对应版本的 GeckoLib 4；在正式引入依赖前需分别确认两个目标节点的兼容版本。核心状态机不从动画回调决定伤害；时间轴先由服务端运行，动画事件只触发表现，以避免低 TPS、停服或客户端改包改变结算。
...
| 掉落 | 数量 | 用途 |
| --- | ---: | --- |
| `god_and_lord_remembrance` 神与王的追忆 | 1 | 制作路线的唯一核心材料 |
| `young_lion_greatsword` 年轻狮子大剑 | 配方解锁 | 临时值：11 攻击伤害、0.90 攻速、2300 耐久、14 附魔能力 |
| `circlet_of_fading_light` 渐隐光冠 | 配方解锁 | 临时值：3 护甲、3.5 盔甲韧性、0.11 击退抗性、450 耐久；圣属性输出 ×1.10 |
| `gate_fragment` 神门残片 | 4-8 | 竞技场装饰和再次挑战召唤物 |

表中的武器攻击伤害指物品栏显示总值，不是额外属性修饰值。以上装备数值仅为高于原版终局装备约 10%-15% 的实现占位值，必须保留配置入口，后续平衡可以替换。配方成本、主动技能和重复挑战方式仍需单独确认。
...
| 四人完整战 | 生命按 4 人档锁定；目标至少发生 3 次有效切换；离场不能降低当前生命 |
| 阶段阈值 | 跨过 65% 时当前有效段正常收招，转换只触发一次 |
...
| 大荒星陨 | 25% 首次强制触发；警示后落点不追踪；核心与外环不重复命中 |
| 场地构造 | 灰白荒原、北侧神之门、南侧雾门、场心与回归锚点符合构造规范；任意 5×5 格高差不超过 1 格 |
| 破坏与穿墙 | 只破坏白名单方块；神之门和边界不受影响；穿墙不离场、不穿盾牌面，重置后恢复被破坏方块 |
| 台词字幕与战吼 | 拉塔恩只保留非语言战吼；米凯拉只在转阶段、第二阶段首次击败玩家和战败事件显示字幕；普通技能无台词；重连不重复；无需对白录音或口型 |
| 字幕与声音独立 | 客户端关闭全部声音时仍显示台词字幕；服务端关闭 `[dialogue]` 时不显示台词，但拉塔恩战吼、受击声和技能提示仍按 `[nonverbal_audio]` 运行 |
...
- 召唤流程与竞技场在世界中的生成位置。
- 临时装备主动技能、配方成本与最终平衡数值。
- 是否要求 Epic Fight、Better Combat 等第三方战斗模组兼容。
