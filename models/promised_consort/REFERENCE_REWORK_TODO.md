...
## 2026-09-18持续四圈、前扑与前空翻

本批三项反馈已实装。动画SHA256 `160bbb2cf3d861a16e3095939a6ac7c06d48850d93030448d4424a6462835a50`，工程 `fbf84df58a06d546b75c89f1356bb145bb77a08a4c89b613b1775b1e4f02289b`。当前复查入口 `node models/promised_consort/scripts/refine_movement_feedback.js --aerial-followthrough --check`；旧加速报告保留为历史，不重新应用。

| 反馈 | 当前实现 |
| --- | --- |
| 重力跃斩持续3至6圈、落地收势 | 选4圈，制作12至38tick绕竖直轴连续转动，直到38落地才停；展开双刀旋转，最后6tick直接压入落地收势，去除独立十字蓄力。普通/远程从旧贴地靠近改为同一真实飞行执行器，冻结落点，0.25格碰撞步进、每tick最多2格，落地再次检查顶面支撑。原剑击和冲击伤害事件保留且都要求已落地，不新增独立终结攻击 |
| 跃进大幅前扑 | 水平曲线改为起跳即有前向速度，近距离也有至少6格前扑意图，目标前1.5格停靠；距离仍按正常16格/远程配置及速度预算收敛。弧高不超过水平距离的16%，保留原2格上限；预检、执行、预警预测同用新路径，普通与远程都生效。无有效落点或障碍阻挡时取消，不强行穿越 |
| 狮子斩前空翻 | 普通与衍生都改为制作-X轴前翻，头部先朝身前下方旋转，与向前斩地势头一致；10至32、8至26tick各完成一圈。实际离地时刻分别对齐制作10/8tick，保留斩地、落地接触及尾段姿态；开场沿用同一起跳锚点 |

仅修改三段主动画，其余41段、防御库、模型与贴图保留；跃进主要修复实体位移，不再次变更其已验过的起跳和落刀姿态。三段最大半tick插值变化约29.46/12.27/15度。普通及开场默认飞行高度叠加后，狮子斩落地前刀尖最低约-1模型单位（原接触深度），脚部标记高于6模型单位；不宣称解决旧收刀尾段低点或任意配置、真实地形下全部穿插。

通过：双平台编译与资源处理；当前资产3005605项；GeckoLib44段1475175值；7872组追步支撑；ActionDebug21046项；Timeline79667项；AttackPlan781630项/4320打击；远程回归；49种合法阶段逐刀音效及双平台音效字节一致。新测试覆盖近距离前扑首步水平位移大于升高2倍、普通/远程跃斩全段腾空、起跳/落地时钟和冻结地面预警。

本轮仅查看一张73964字节20格二阶段拼图，叠加默认解析实体路径，包含三招和狮子斩衍生。可见前翻、前移低弧及四圈末段直接收势；模型较小，静态样本不证明完整连续速度、真实碰撞、网络或附身穿插。图片已删除，没有保存临时模型文件；未操作Minecraft、未提交或推送Git。全招原片细查仍是另一个未完成工作。

## 2026-09-18全部挥刀加速与横向跃斩

本轮两项要求已实装。主动画SHA256 `49883f38dc4ea675e844541db7b261e771c6c1a18e015eb396a7e0f3eb606e19`，防御动画 `0010fcf56ef17446c9676895190a3fda2685e1c1145714f23dc2f9499e89304a`，制作工程 `7e221f82a6eb0e53b090427924281ae9af2a17c909a86f9f61687c9c20fa3c14`。当前复查入口 `node models/promised_consort/scripts/refine_movement_feedback.js --all-slashes --check`；下方旧修订及其哈希只作历史记录。

- 覆盖20段本体、6类分身与3类独立防御动作，共48个本体挥刀窗口、6个分身释放、3个防御拨刀释放。上一轮已加速的8个窗口保留，不重复叠加；实际新增修改21段主库与3段防御，23段主库完全不变。待机、移动、持架和无挥刀施法不压缩。
- 默认二次加速；右起交叉、右起左双斩、王者连舞分别使用1.8/1.55/1.4次幂，避免局部超速。躯干、手臂与腿部使用同一姿态时钟，保留接触姿态、原命中时刻、后摇终点、总时长和伤害。57窗口真实刀尖末四分之一行程均超过前四分之一的1.5倍；1/16tick插值检查沿用本体65度/半tick、分身95度/半tick上限，原有更快通道不得新增超速。
- 重力跃斩 `gravity_dive` 在制作12至35tick绕竖直Y轴转360度，移除骨盆空翻俯仰及前后轴翻转；35tick转正十字蓄势、38tick落刀和79tick总长保留。不改变实体飞行路径或新增旋转伤害。
- 工程保持唯一当前版本，90777056字节，低于100MiB；删除数值共线冗余帧后，对全部原采样点的线性插值误差断言小于0.00001。没有旧模型副本。

验证通过：当前资产2984899项；实际GeckoLib主库44段1464822变换值、防御3段63693值；7872组追步支撑；Timeline79667项；49种合法阶段逐刀音效合同、48接触/76本体刀声；双平台资源处理与主库、防御库、音效字节一致。本轮未改Java玩法代码，不把旧编译/攻击测试记为新运行。

只查看一张78569字节、12格二阶段静态拼图，重力跃斩12/16/20/24/28/31/35/38时刻显示直立横转，再接十字与落地压身；另含两段连斩蓄势/接触姿态。Blockbench185个空中样本胸腔朝上分量最低0.866026。临时图已删除，没有保存新模型；静态小图不等于全部连续动作、穿插或世界验收。未操作Minecraft、未提交或推送Git。更早的逐招原片细查工作仍未完成。

## 2026-09-18加速斩击、侧砍与阶段保护

本批反馈已接入当前唯一版本。动画SHA256 `f125ce32f6f2f455998aca50a2facbd9cd4fbd4cc9210ffc81f3f1e2f1728765`，工程 `68e8d3ffc2e9b11747e0f05c338a422469df6bc14c6c97ec6fd2ff07db75805c`。当前复查入口为 `node models/promised_consort/scripts/refine_movement_feedback.js --slash-release --check`；旧动作报告和下方历史哈希不代表当前资源，不应重新应用旧修订。

制作工程关键帧统一为每帧一行，将143493436字节压至68005536字节，低于GitHub单文件100MiB限制；写入前已对整个解析对象做严格相等检查，没有删减关键帧或改变任何姿态，运行时动画字节不变。

| 反馈 | 当前实现 |
| --- | --- |
| 陨石侧方砍地 | 身体侧转、双刀同侧落地，权威扇区按模型到实体朝向转换，与预警和真实伤害共用；保留原伤害值、命中时刻 |
| 强化掀地、光速斩本体/分身、唤星终斩 | 延长有效蓄势，末段非匀速加速释放，原接触时刻及总时长保留；直接检查真实刀尖轨迹，不只核对映射公式 |
| 跃进、重力劈跃 | 落地前真实刀段交叉，放宽落刀、胸腹更深下压、骨盆额外降低3模型单位；保留原脚位与脚掌方向。跃进第二刀40tick回刀后加速压至43接触 |
| 跃进起跳 | 制作12→7tick，普通/远程实体路径与动画时钟一起调整；增加腾空时间占比，两次接触仍35/43、总长79 |
| 陨石空中预警 | 隐藏 `rock_flight*` 和 `clone_meteor_*` 范围包，保留真实打击计划、开场侧砍和最终本体落地预警 |
| 大伤害转阶段 | 活跃一阶段生命写入、tick检查、硬直分支及死亡兜底均保护转阶段；默认65%不变，配置再低也至少5%，不等死亡才触发。管理员强制清除和单招测试除外 |

七段改动，其余37段、模型与贴图保留。当前1/16tick插值检查通过；本批快速释放门槛明确高于旧45度起手诊断，实际最大半tick等效关节变化约58.09度。已有光速斩/唤星刀尖轻微低于模型地面仍记录为既有边界，不冒充全招触地质量验收。

通过：双加载器编译与资源处理；当前资产2205161项；GeckoLib44段1074954变换值；7872组追步支撑；ActionDebug20878项；Timeline79667项；AttackPlan781628项/4320打击；远程反制回归；49种合法阶段逐刀音效及双平台音效字节一致性。攻击预警断言减少来自明确隐藏空中提示，命中次数未减少。

只查看一张57803字节、12格二阶段静态拼图，覆盖侧砍、十字蓄势和落地压身；已删除临时图，保留原Blockbench工程中的44段动画，不保存临时审查副本。未重新逐招分析原片，也未操作Minecraft。真实实体超额伤害、直接清血、硬直及读档后的转阶段，完整动态穿插、飞行位移和听感仍需世界验收；下方全招原片细查任务仍未完成。

## 2026-09-18完整跃击与材质重做

本批六项反馈已接入当前版本，动画SHA256 `27bd92fbd1531d5a726a8af8e3770b4c97b5588aa02b638e69d0ff76e1b329f0`，制作工程 `e29e9276d2eea104a3a1dbba99849bebadb41932acf84d2aa12737bd5f0a92e8`。下方旧版描述不作为当前实现。

| 反馈 | 当前实现及边界 |
| --- | --- |
| 狮子斩落点靠后 | 沿本次来袭方向瞄准玩家后方1.5格，不再减去前方停靠距离；正常飞行上限2格/tick、仍用0.25格碰撞步进。最大距离、支撑面、墙体和场地边界继续约束，超出可达距离时不承诺越过目标 |
| 完整重置跃击 | 不再拼接其他攻击姿态。独立制作下蹲蓄力0至10、蹬地10至12、空中收腿12至28、提前落刀28至35、落地压身35至37、短回收与第二刀37至43、余势恢复43至79。脚部IK配合骨盆，起跳前双脚保持，伤害仍为制作35/43两接触；其他43段保留 |
| 陨石准备节奏 | 旧默认首发前摇86→50运行tick。默认砍地12、停3tick、起飞15、到顶31、悬浮瞄准至首发50；原投石节拍和数量保留，整体缩短36tick。悬浮至落地锁定前持续朝向本动作的锁定目标；目标失效不换人瞄准。三份配置只迁移旧默认86，自定义值保留，不生成旧快照 |
| 纯净圣光 | 去除法阵辐条、网格、重复环纹和环墙，改中心光柱、外围光柱与柔和低透明外晕，地面散光约1.5倍视觉范围、柱体外晕约3.2倍宽度。是着色器散光，不是方块光照或额外伤害；独立玩法预警保持 |
| 破碎火焰 | 火舌柱和连续火焰片不再走生产渲染，改离散火焰粒子与少量烟，独立上升、寿命10至24tick。粒子分支不依赖能量着色器成功加载；数量预算和距离限制仍保留，不影响火焰伤害 |
| 真实踏地 | 跳过通用金色能量/冲击层，按前方波阵连续8tick喷出真实地面方块材质碎粒，大小2至3倍、寿命24至40tick。默认单Boss每tick预算18，理论上最多144颗，实际数量受地面有效性、粒子设置及共享预算影响；不破坏方块 |

新跃击15个控制姿态、1/8tick求解与1/16tick检查：起跳前最大脚位误差0.007389模型单位，最大半tick等效关节变化33.820623度，刀尖最低约-0.600002模型单位。已断言蓄力到蹬地骨盆上升超过7模型单位、空中双脚收高超过9模型单位、落地前刀刃下降、落地头胸压低。只查看一张102904字节临时图，8时刻/两阶段16格，能区分蓄力、蹬伸、收腿与压身；不包含实体移动或完整表面穿插验收。临时副本移除，图像按要求删除。

通过：双加载器离线编译；当前资产1604603项；GeckoLib44段774675变换值；7872组追步支撑；ActionDebug20820项；Timeline79667项；AttackPlan804770项/4320打击；远程回归；49种合法阶段音效合同；GPU8模式16帧，纯光中心到外晕单调衰减且外晕可见。测试数减少来自陨石总时长缩短，不是移除命中或放松边界。仍需用户在世界中复测落点、动作力度、悬浮跟随、光效及颗粒观感，未自动启动客户端。

## 2026-09-18陨石提速与跃进替换

本批五项追加反馈已实装，当前动画SHA256 `68196ddc29cf8b5187a7032b1c9caf0505e53a8c821004df68c842ad4d45a6ce`，制作工程 `27099a3ec863d2b2d97d789b2496955450df79015956c0d2a42dda6f08724f98`。当前模型与贴图不变；只替换原螺旋突进动画，其他43段保留。

| 反馈 | 当前实现 |
| --- | --- |
| 重力陨石飞行更快并配合挥刀 | 上升对齐制作28tick上挑开始、45tick举刀到位；默认运行起飞27/到顶43，原34/61，约8格前移与升高用16tick完成，原27tick。石块聚拢同步使用该上升段时长并保存，旧存档缺项默认16tick。掀石与投石命中时刻、伤害保留 |
| 连斩判定扩大 | 六类左右连斩、王者连舞、十字跃袭的本体刀击额外扩大15%，从共享攻击计划生成实际判定与预警。0.8格后缘不改；血焰爆发、分身、圣光余波不跟随该倍率 |
| 螺旋突进改跃进 | 显示名“跃进”，保留 `spiral_assault` 和 `spin_damage`/`slam_damage` 兼容键。普通最多16格接近，远程按原最大距离配置及2格/tick飞行预算收敛；低弧高约2格，起跳锁点后才离地，落地后保留两次攻击。新79tick动画35/43接触不变，躯干无整圈旋转；起跳和启动声对应制作12tick。狮子斩目标弧高提升至6格，短时序根据垂直/水平速度预算收敛，不强制超速 |
| 碎石更明显 | 真实刀刃触地颗粒放大1.6倍，提高散布和抛射幅度；原每刀/窗口去重、真实碰撞与粒子数量预算保留，不凭空制造地面接触 |
| 陨石更大且特效同比放大 | 统一 `SIZE_SCALE=1.35` 驱动物品渲染、碰撞尺寸、环绕间距、光晕、折射半径与尾迹；碰撞宽高从0.75增至1.0125格。数量、伤害、耐久不变；更宽的实体仍受环境碰撞约束 |

离线验证：ActionDebug21015项（普通/远程低弧路径、冻结落点、两命中、起跳姿态/声音、高弧速度、范围和尺寸）；Timeline79811项；AttackPlan804770项/4320打击；远程配置/路径/NBT/指示器回归；49种合法阶段逐刀音效合同；实际GeckoLib44段774675值及7872组追步支撑；当前资产1604603项；双加载器编译、资源处理通过。

仅查看一张临时跃进图：118731字节、8个制作时刻0/12/22/30/35/39/43/60、两阶段16格；Blockbench159个半tick姿态检查胸腔未倒置。图中可见举刀前倾、落地低斩和短回收，没有旧整圈旋转；不包含实体位移或完整穿插检查，不能替代游戏中的低弧线验收。临时动画副本已移除、图片查看后删除；未操作Minecraft，也未将此前客户端运行当成本批验收。

## 2026-09-18七项实机反馈修正

本批已按当前唯一版本增量实装，未改伤害值、命中次数或阶段时长。当前动画SHA256为 `1b37f549bd211d401f7ad07afadd47de09d31dee88d145cda5b8a3cd765cb0ba`，制作工程为 `349318b9c264171265abc09fe45de6557d40e877890363bc5b155c198a0124b8`。下方更早版本和截图说明仅为历史文字。

| 反馈 | 本批实装 | 验证边界 |
| --- | --- | --- |
| 1 重力陨石掀石和起飞 | 双刀侧方低扫、向前上方带刀；服务端沿锁定出刀朝向斜升，前方高点保存于动作状态，随后保留投石与落地流程 | 主视频155.8至157.8秒可见起手，完整起飞不在本次图内；飞行方向另据用户明确反馈，不宣称精确原片3D复刻 |
| 2 直线冲刺自动上坡 | 普通直线移动/重力接近、直线光速突贯及四远程冲刺沿地面支撑逐步移动，先抬高再前移；路线预检不再强制等高 | 单步上升至多1格、下降至多1.25格；脚部包围盒提前探测台阶边；保留流体、净空、区块和场地边界检查 |
| 3 重力更大且不稳定 | Boss折射半径基值1.6→2.6、岩石0.6→0.85；多频有界起伏、扭曲螺旋和断续能量纹，岩石光晕同步扩大 | 只扩大表现，不扩大伤害；GPU动态像素及零强度/范围外保护通过，实机观感未验收 |
| 4 光速侧闪斜前方 | 普通与远程侧闪都使用朝向与侧向组合的单位对角方向，保留原总移动预算 | 四朝向方向测试及远程路径回归通过 |
| 5 大荒星陨落地裂地/爆炸 | 真实落地状态触发径向裂缝与既有爆发、冲击圈；落地同时播放独立石裂和爆炸层，复用已有音色 | 不增加伤害或删除方块；不按渲染帧重复播放；未进行游戏内听评 |
| 6 狮子斩被小坡挡住 | 后半段改为更高净空的弧线，落点按整个脚部支撑面修正，小台阶采用抬高再前移而非立即取消 | 不是无限穿墙；大型障碍、速度超限或没有安全落点仍可取消 |
| 7 狮子斩起手刀路 | 普通/衍生两段举刀改为身前高位，向前下方入地；四元数连接起手姿态，消除不合适的欧拉角绕行 | 原接触帧本身已在身前；修正的是举刀和过渡路径。新模型连续观感/穿插尚未目视验收 |

三段动画仅修改起手0至63/14/12制作tick，保留41段其他动画、脚步、手腕握持及后续飞行/落地空间姿态。2163个1/8tick样本通过，最大半tick关节旋转分别19.571/25.009/33.209度，地面段最低刀尖Y约-0.871，未加新命中。

离线通过：双加载器编译与资源处理；当前资产1537283项；实际GeckoLib44段741015个变换值；7872组追步支撑；ActionDebug20427项；Timeline79811项；远程路径/选择/指示器回归；49种合法阶段音效合同、48接触/76刀声；能量着色器7模式14帧和重力折射GPU像素测试。测试不是完整世界碰撞、多人、保存重进或视觉听感验收。

本轮只查看一张847273字节临时参考合图：主视频左40样本150至158秒、右40样本162至170秒，每秒5帧；包含前一招余势、远景和末尾黑场，不能把80样本都记成有效起手原帧。截图在检查后删除，需要时重新生成。未操作Minecraft，未提交或推送本批修改。
